...
def url(message):
    ...
    viki_malinovi = types.InlineKeyboardButton(text='вот о малиновом чае!', url='https://teawiki.ru/chai/malinovyj-polza-i-vred/')
    markup.add(viki_malinovi)
    bot.send_message(message.chat.id, "Тыкай, там о малиновом чае <3", reply_markup=markup)
@bot.message_handler(func=lambda m: m.text =="зеленый чай")
def url(message):
    markup = types.InlineKeyboardMarkup()
    viki_zeleniy = types.InlineKeyboardButton(text='вот о зеленом чае!', url='https://ru.wikipedia.org/wiki/%D0%97%D0%B5%D0%BB%D1%91%D0%BD%D1%8B%D0%B9_%D1%87%D0%B0%D0%B9')
    markup.add(viki_zeleniy)
    bot.send_message(message.chat.id, "Тяпай, там о зеленом чае <3", reply_markup=markup)
